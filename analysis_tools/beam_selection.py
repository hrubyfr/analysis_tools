...
class SelectionMonitor:
    ...
    T5_X_RANGE = (-150.0, 150.0)
    T5_Y_RANGE = (-68.15, 68.15)
    T5_X_BINS = 50
    T5_Y_BINS = 8
    T5_PARTICLE_ORDER = ["electron", "muon", "pion", "proton"]
    ...
    def __init__(self, selections, update_every=10, bins=100, vme_run_info=None):
        ...
        self._t5_sel_xy_counts  = {sel.name: None for sel in selections}
    ...
    def _accumulate(self, batch, masked_batches):
        ...
        if "T5_hit_pos_x" in ak.fields(batch) and "T5_hit_pos_y" in ak.fields(batch):
            x_edges = np.linspace(self.T5_X_RANGE[0], self.T5_X_RANGE[1], self.T5_X_BINS + 1)
            y_edges = np.linspace(self.T5_Y_RANGE[0], self.T5_Y_RANGE[1], self.T5_Y_BINS + 1)
            for sel_name, sel_batch in masked_batches.items():
                if ("T5_hit_pos_x" not in ak.fields(sel_batch)
                        or "T5_hit_pos_y" not in ak.fields(sel_batch)):
                    continue
                sel_x = ak.to_numpy(ak.flatten(sel_batch["T5_hit_pos_x"], axis=None))
                sel_y = ak.to_numpy(ak.flatten(sel_batch["T5_hit_pos_y"], axis=None))
                sel_valid = np.isfinite(sel_x) & np.isfinite(sel_y)
                sel_x, sel_y = sel_x[sel_valid], sel_y[sel_valid]
                if len(sel_x) == 0:
                    continue
                if self._t5_sel_xy_counts[sel_name] is None:
                    self._t5_sel_xy_counts[sel_name] = np.zeros(
                        (self.T5_X_BINS, self.T5_Y_BINS), dtype=np.float64
                    )
                sel_counts, _, _ = np.histogram2d(
                    sel_x,
                    sel_y,
                    bins=(x_edges, y_edges),
                    range=[self.T5_X_RANGE, self.T5_Y_RANGE],
                )
                self._t5_sel_xy_counts[sel_name] += sel_counts
    ...
    def show(self):
        ...
        t5_names = [
            name for name in self.T5_PARTICLE_ORDER if name in self._t5_sel_xy_counts
        ]
        t5_names.extend(
            name for name in self._t5_sel_xy_counts
            if name not in self.T5_PARTICLE_ORDER
        )
        n += len(t5_names)
        if n == 0:
            return
        ...
        if t5_names:
            x_edges = np.linspace(self.T5_X_RANGE[0], self.T5_X_RANGE[1], self.T5_X_BINS + 1)
            y_edges = np.linspace(self.T5_Y_RANGE[0], self.T5_Y_RANGE[1], self.T5_Y_BINS + 1)
            t5_axes = axes[len(self._variables):]
            for ax, sel_name in zip(t5_axes, t5_names):
                counts = self._t5_sel_xy_counts[sel_name]
                if counts is None:
                    counts = np.zeros((self.T5_X_BINS, self.T5_Y_BINS), dtype=np.float64)
                im = ax.pcolormesh(
                    x_edges,
                    y_edges,
                    np.log1p(counts).T,
                ...
                ax.set_xlim(*self.T5_X_RANGE)
                ax.set_ylim(*self.T5_Y_RANGE)
                ax.set_xlabel("T5_hit_pos_x")
                ax.set_ylabel("T5_hit_pos_y")
                ax.set_title(f"T5 hit position: {sel_name} ({self._sel_total[sel_name]:,})")
